...
$error_list = array();
if ($part == 'h')
    $result = mysql_query("SELECT net, bhs AS low, word, bhsstr_word.strong, bhsstr_word.kata, tipe, stage FROM giza_tl_linkage_clean LEFT JOIN giza_tl_pairmap USING (pair) LEFT JOIN pltl_word ON giza_tl_pairmap.ayat = pltl_word.id AND giza_tl_linkage_clean.net = pltl_word.pos LEFT JOIN bhsstr_word ON giza_tl_pairmap.ayat = bhsstr_word.ayat AND giza_tl_linkage_clean.bhs = bhsstr_word.pos WHERE giza_tl_pairmap.ayat = $id ORDER BY net, bhs;");
else
