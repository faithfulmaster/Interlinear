...
$error_list = array();
if ($part == 'h')
    $result = mysql_query("SELECT net, bhs AS low, tanda, bhsstr_word.strong, bhsstr_word.kata, catatan, tipe, stage FROM giza_ot_linkage_clean LEFT JOIN giza_ot_pairmap USING (pair) LEFT JOIN net_chop ON giza_ot_pairmap.ayat = net_chop.ayat AND giza_ot_linkage_clean.net = net_chop.pos LEFT JOIN bhsstr_word ON giza_ot_pairmap.ayat = bhsstr_word.ayat AND giza_ot_linkage_clean.bhs = bhsstr_word.pos WHERE giza_ot_pairmap.ayat = $id ORDER BY net, bhs;");
else
    $result = mysql_query("SELECT net, wh AS low, tanda, wh_word.strong, wh_word.kata, catatan, tipe, stage FROM giza_linkage_grk2net LEFT JOIN net_chop ON giza_linkage_grk2net.ayat = net_chop.ayat AND giza_linkage_grk2net.net = net_chop.pos LEFT JOIN wh_word ON giza_linkage_grk2net.ayat = wh_word.ayat AND giza_linkage_grk2net.wh = wh_word.pos WHERE giza_linkage_grk2net.ayat = $id ORDER BY net, wh;");
while ($row = mysql_fetch_assoc($result)) {
    $node = array('net' => $row['net'], 'low' => $row['low'], 'word' => $row['tanda'], 'strong' => $row['strong'], 'lower' => $row['kata'], 'notes' => $row['catatan'], 'tipe' => $row['tipe'], 'stage' => $row['stage']);
    $break = FALSE;
