...
echo "<form>\n";
echo "<input type=\"text\" value=\"$word\" name=\"w\">\n";
echo "</form>\n";

$result = mysql_query("SELECT COUNT(*) AS count FROM pbtb_word WHERE word = '$word';");
if ($row = mysql_fetch_assoc($result))
    $total = $row['count'];

$result = mysql_query("SELECT ayat, book, chapter, verse, wh, strong, COUNT(*) AS count, net, tipe, stage FROM $linkage_table LEFT JOIN verses ON ayat = verses.id WHERE kata = '$word' GROUP BY ayat, net ORDER BY count;");
while ($row = mysql_fetch_assoc($result)) {
    if ($row['count'] > 1) {
        $i = 0;
        $word_list = array();
        $pos_list = array();
        $result2 = mysql_query("SELECT wh, strong, tipe, stage FROM $linkage_table WHERE kata = '$word' AND ayat = {$row['ayat']} AND net = {$row['net']} ORDER BY wh;");
        while ($row2 = mysql_fetch_assoc($result2)) {
            if (!is_null($row2['strong'])) {
                if (($row2['wh'] - $i > 1) && ($i != 0))
                    $word_list[] = str_repeat('.', $row2['wh'] - $i) . '.';
                $i = $row2['wh'];
                if ($row2['tipe'])
                    $word_list[] = "<a href=\"strong.php?s={$row2['strong']}\">#</a><span class=\"t{$row2['tipe']}\">{$row2['strong']}</span>";
                else
                    $word_list[] = "<a href=\"strong.php?s={$row2['strong']}\">#</a>{$row2['strong']}";
                $pos_list[] = $row2['wh'];
            }
        }
        if (count($word_list))
        ...
        $rel_list = array();
        $result2 = mysql_query("SELECT DISTINCT net, kata, tipe FROM $linkage_table WHERE ayat = {$row['ayat']} AND wh IN ($posi) AND kata != '$word' ORDER BY net;");
        while ($row2 = mysql_fetch_assoc($result2)) {
            if ($row2['tipe'])
                $rel_list[] = "<span class=\"t{$row2['tipe']}\">{$row2['kata']}</span>";
            else
                $rel_list[] = "{$row2['kata']}";
        }
        if (count($rel_list))
        ...
        if (array_key_exists($phrase, $to_many[$row['stage']]))
            $to_many[$row['stage']][$phrase][$row['ayat'] * 10000 + $row['net'] * 100 + $net] = array($row, implode('|', $pos_list));
        else
            $to_many[$row['stage']][$phrase] = array($row['ayat'] * 10000 + $row['net'] * 100 + $net => array($row, implode('|', $pos_list)));
    }
    elseif (!is_null($row['strong']) && ($row['strong'] != '')) {
        if ($row['tipe'])
            $strong = "<a href=\"strong.php?s={$row['strong']}\">#</a><span class=\"t{$row['tipe']}\">{$row['strong']}</span>";
        else
            $strong = "<a href=\"strong.php?s={$row['strong']}\">#</a>{$row['strong']}";
        $rel_list = array();
        $result2 = mysql_query("SELECT DISTINCT net, kata, tipe FROM $linkage_table WHERE ayat = {$row['ayat']} AND wh = {$row['wh']} AND kata != '$kata' ORDER BY net;");
        while ($row2 = mysql_fetch_assoc($result2)) {
            if ($row2['tipe'])
                $rel_list[] = "<span class=\"t{$row2['tipe']}\">{$row2['kata']}</span>";
            else
        ...
        if (count($rel_list))
            $strong .= ' [' . implode(' ', $rel_list) . ']';
        if (!array_key_exists($row['stage'], $to_one))
            $to_one[$row['stage']] = array();
        if (array_key_exists($strong, $to_one[$row['stage']]))
            $to_one[$row['stage']][$strong][$row['ayat'] * 10000 + $row['net'] * 100 + $row['wh']] = $row;
        else
            $to_one[$row['stage']][$strong] = array($row['ayat'] * 10000 + $row['net'] * 100 + $row['wh'] => $row);
    }
    elseif (!is_null($row['strong'])) {
        if (!array_key_exists($row['stage'], $to_zero))
            $to_zero[$row['stage']] = array();
        $to_zero[$row['stage']][$row['ayat'] * 100 + $row['net']] = $row;
    }
    else {
        if (!array_key_exists($row['stage'], $to_none))
            $to_none[$row['stage']] = array();
        $to_none[$row['stage']][$row['ayat'] * 100 + $row['net']] = $row;
    }
    if ($row['stage'] > $max_stage)
        $max_stage = $row['stage'];
}

$result = mysql_query("SELECT * FROM $linkage_table WHERE kata = '$kata' AND stage = 0;");
        ...
        if (count($to_one_index)) {
            ...
            foreach ($to_one_index as $key => $value)
                foreach ($value as $val) {
                    ...
                    $combo = array();
                    foreach ($refs as $ref)
                        $combo[] = $ref['ayat'] * 100 + $ref['net'];
                    $combo = implode(',', $combo);
                    if (!$stage)
                        array_push($super_combo, $combo);
                    echo "<td><a href=\"$combo_page?ids=$combo&amp;w=$word\">$key</a></td><td>";
                    $auto++;
                    $i = 1;
                    foreach ($refs as $ref) {
                        echo "<a href=\"$detail_page?id={$ref['ayat']}&amp;w=$word\" title=\"{$books[$ref['book']]['abbr']} {$ref['chapter']}:{$ref['verse']}\">&middot;</a>";
                        if ($i % 10 == 0)
                            echo "\n";
                        $i++;
                    }
        ...
        if (count($to_many_index)) {
            ...
            foreach ($to_many_index as $key => $value)
                foreach ($value as $val) {
                    ...
                    $combo = array();
                    foreach ($refs as $ref)
                        $combo[] = $ref[0]['ayat'] * 100 + $ref[0]['net'];
                    $combo = implode(',', $combo);
                    if (!$stage)
                        array_push($super_combo, $combo);
                    echo "<td><a href=\"$combo_page?ids=$combo&amp;w=$word\">$key</a></td><td>";
                    ...
                    $i = 1;
                    foreach ($refs as $ref) {
                        echo "<a href=\"$detail_page?id={$ref[0]['ayat']}&amp;w=$word\" title=\"{$books[$ref[0]['book']]['abbr']} {$ref[0]['chapter']}:{$ref[0]['verse']}\">&middot;</a>";
                        if ($i % 10 == 0)
                            echo "\n";
                        $i++;
                    }
...
//                        mysql_query("INSERT INTO $found_table VALUES ($strong, \"$v\", $key);");
    ...
    if (count($to_zero[$stage])) {
        ...
        if (!$stage)
            array_push($super_combo, $combo);
        echo "<td><a href=\"$combo_page?ids=$combo&amp;w=$word\">$count</a></td><td>";
        $auto++;
        $i = 1;
        foreach ($to_zero[$stage] as $ref) {
            echo "<a href=\"$detail_page?id={$ref['ayat']}&amp;w=$word\" title=\"{$books[$ref['book']]['abbr']} {$ref['chapter']}:{$ref['verse']}\">&middot;</a>";
...
if ($super_combo && count($super_combo)) {
    ...
    echo "<a href=\"$combo_page?ids=$super_combo&amp;w=$word\">COMBO EDITOR</a>\n";
}
/*
if ($unchecked)
...
if ($total) {
    $leftout = array();
    $result = mysql_query("SELECT pbtb_word.id as ayat, pbtb_word.pos FROM pbtb_word LEFT JOIN $linkage_table ON pbtb_word.id = $linkage_table.ayat AND pbtb_word.pos = $linkage_table.net WHERE pbtb_word.word = '$word' AND $linkage_table.ayat IS NULL AND $linkage_table.net IS NULL;");
    while ($row = mysql_fetch_assoc($result)) {
